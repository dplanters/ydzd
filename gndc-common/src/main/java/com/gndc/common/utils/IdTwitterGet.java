...
/**
 * from
 * https://github.com/twitter/snowflake/blob/scala_28/src/test/scala/com/twitter/service/snowflake/IdWorkerSpec.scala
 *
 * @author xiaomo (hcxxiaomo@gmail.com)
 * @since 0.1
 * <p>
 * Twitter的snowflake算法，ID组成为
 * 毫秒级时间(41位)+机器ID(10位，数据中心(5位)+节点(5位))+毫秒内序列(12位). 时间戳则是记录了从
 * 1288834974657 (Thu, 04 Nov 2010 01:42:54 GMT)
 * 这一时刻到当前时间所经过的毫秒数，41位，符号位为0
 */
public class IdTwitterGet {
    ...
    private synchronized long nextId() {
        ...
        /*
         * long id = ((timestamp - defaultTimestamp) << timestampLeftShift)//前41位 +
         * (datacenterId << datacenterIdShift)//中间前5位 + (workerId <<
         * workerIdShift)//中间后5位 + sequence;//最后12位
         */
        return id;
    }
